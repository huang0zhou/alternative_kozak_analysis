# https://ftp.ncbi.nlm.nih.gov/refseq/M_musculus/annotation_releases/GCF_000001635.27-RS_2024_02/
import pandas as pd
import re
...
def extract_exon_info(header):
    match = re.search(r'location=(complement\()?join\(([^)]+)\)\)?', header)
    if match:
        is_complement = match.group(1) is not None
        exon_ranges = match.group(2).split(',')
        exons = []
...
def main():
    ...
    for header, dna_seq in sequences.items():
        ...
        if positions:
            exons = extract_exon_info(header)
            for pos in positions:
                exon_number = get_exon_number(pos, exons)
                total_length = len(dna_seq)
                start_index = max(0, pos - 15)
                substring = dna_seq[start_index:pos + 4]
                results.append((header, total_length, substring, pos, exon_number))
    ...
    output_file = "/mnt/public/huanghz/ORF/cDNA_exon_with_alternative_kozak_analysis_results_shrame_shift_or_not_true_exon.csv"
